...
module_path = r'C:\Users\Gigabyte\Downloads\create.py'

try:
    # Dynamically import the module using importlib
    spec = importlib.util.spec_from_file_location("create", module_path)
    create = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(create)
except Exception as e:
    print(f"An error occurred: {e}")
    # Example: Installing requests package if it fails (though this doesn't seem necessary here)
    os.system('pip install requests')
    # You might want to handle the import of `create.py` again after installing the package or handle other exceptions as needed

try:
    from bs4 import BeautifulSoup as bs # type: ignore
except ImportError:
    # ImportError is raised if the bs4 module is not installed
...
class create_fb:
    ...
    def __init__(self):
        ...
        self.abc = request.Session() #--> Sesi Email
        self.xyz = request.Session() #--> Sesi Facebook
    ...
    def get_email_cryptogmail(self):
        ...
        dom = random.choice(['temp-mail.io/en','chitthi.in','fextemp.com','any.pink','merepost.com'])
